...
// A(_XOPEN_SHM >= 0)
 A(_XOPEN_UNIX >= 0)
#endif
#ifdef __wasilibc_unmodified_upstream // WASI has no aio
A(_POSIX_ASYNCHRONOUS_IO >= 200809L)
#endif
A(_POSIX_BARRIERS >= 200809L)
A(_POSIX_CHOWN_RESTRICTED >= 0)
A(_POSIX_CLOCK_SELECTION >= 200809L)
#ifdef __wasilibc_unmodified_upstream // WASI has no job control
A(_POSIX_JOB_CONTROL > 0)
#endif
#ifdef __wasilibc_unmodified_upstream // WASI has no mapped files
...
A(_POSIX_REGEXP > 0)
#ifdef __wasilibc_unmodified_upstream // WASI has no getuid
A(_POSIX_SAVED_IDS > 0)
#endif
#ifdef __wasilibc_unmodified_upstream // WASI has no semaphores
A(_POSIX_SEMAPHORES >= 200809L)
#endif
#ifdef __wasilibc_unmodified_upstream // WASI has no shell
A(_POSIX_SHELL > 0)
#endif
...
// not required by the standard
 ...
 A(_POSIX_FSYNC >= 0)
 A(_POSIX_IPV6 >= 0)
#ifdef __wasilibc_unmodified_upstream // WASI has no memlock
 A(_POSIX_MEMLOCK >= 0)
 A(_POSIX_MEMLOCK_RANGE >= 0)
#endif
#ifdef __wasilibc_unmodified_upstream // WASI has no message passing
...
#ifdef __wasilibc_unmodified_upstream // WASI has no spawn
 A(_POSIX_SPAWN >= 0)
#endif
...
#ifdef __wasilibc_unmodified_upstream // WASI doesn't define these
...
C(_PC_SYMLINK_MAX)
C(_PC_SYNC_IO)
#ifdef __wasilibc_unmodified_upstream // WASI doesn't define this
...
#ifdef __wasilibc_unmodified_upstream // WASI has no alarm
{unsigned(*p)(unsigned) = alarm;}
#endif
#ifdef __wasilibc_unmodified_upstream // WASI has no chdir
...
#ifdef __wasilibc_unmodified_upstream // WASI has no exec
...
#endif
#ifdef __wasilibc_unmodified_upstream // WASI has no fork
{pid_t(*p)(void) = fork;}
#endif
{long(*p)(int,int) = fpathconf;}
{int(*p)(int) = fsync;}
{int(*p)(int,off_t) = ftruncate;}
#ifdef __wasilibc_unmodified_upstream // WASI has no getcwd
...
{gid_t(*p)(void) = getgid;}
#endif
#ifdef __wasilibc_unmodified_upstream // WASI has no nss
{int(*p)(int,gid_t[]) = getgroups;}
{int(*p)(char*,size_t) = gethostname;}
#endif
#ifdef __wasilibc_unmodified_upstream // WASI has no getlogin
...
{long(*p)(int) = sysconf;}
#ifdef __wasilibc_unmodified_upstream // WASI has no tcgetpgrp
