...
int __pthread_mutex_trylock_owner(pthread_mutex_t *m)
{
	...
	if (type & 128) {
		if (!self->robust_list.off) {
			self->robust_list.off = (char*)&FUTEX_MAP(m->zzz_m_lock)-(char *)&m->_m_next;
	...
	if (a_cas(&FUTEX_MAP(m->zzz_m_lock), old, tid) != old) {
...
int __pthread_mutex_trylock(pthread_mutex_t *m)
{
	if ((m->_m_type&15) == PTHREAD_MUTEX_NORMAL)
		return a_cas(&FUTEX_MAP(m->zzz_m_lock), 0, EBUSY) & EBUSY;
